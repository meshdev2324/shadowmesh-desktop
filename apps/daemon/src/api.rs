...
pub trait ShadowApi: Send + Sync {
    ...
    /// RFC-018: full pairing session — status plus the minted activation
    /// credential once an authorized device has approved it. Default: not
    /// supported (mocks and legacy backends), the real client overrides.
    async fn qr_status_full(
        &self,
        _token: String,
    ) -> Result<shadowmesh_core::api_client::QrPairingStatus, ShadowMeshError> {
        Err(ShadowMeshError::Other("qr_status_full not supported by this backend".into()))
    }
    /// RFC-018: approve a pairing session with this device's identity.
    async fn qr_authorize(&self, token: String) -> Result<(), ShadowMeshError>;
...
impl ShadowApi for CoreApiWrapper {
    ...
    async fn qr_status_full(
        ...
        self.inner.qr_status_full_async(token).await
    }
    async fn qr_status(&self, token: String) -> Result<String, ShadowMeshError> {
        self.inner.qr_status_async(token).await
    }
    async fn qr_authorize(&self, token: String) -> Result<(), ShadowMeshError> {
        self.inner.qr_authorize_async(token).await
    }
    async fn check_health(&self) -> Result<HealthStatus, ShadowMeshError> {
