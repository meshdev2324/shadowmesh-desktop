...
pub async fn run_daemon_service(daemon: Arc<Daemon>) -> anyhow::Result<()> {
    ...
    security::init_security_protections();
    ...
    // Apply persistent settings on startup
    {
        ...
        if let Some(code) = &config.activation_code {
            ...
            let _ = crate::run_blocking(move || d_v.activate(code_c, auth_t, plan_n, dev_r, rem_d))
                .await;
        }
    ...
    // Security: Check for IPC token from parent process (Mandatory for Release)
    #[cfg(not(debug_assertions))]
    {
        if std::env::var("SHADOWMESH_IPC_TOKEN").is_err() {
            error!(
        ...
        loop {
            let res = std::panic::AssertUnwindSafe(async {
                ...
                if auth_token.is_some() {
                    let stats_res =
                        crate::run_blocking(move || d_hb_core.vpn_manager.get_protocol_stats())
                            .await;
                        ...
                        match hb_res {
                            Ok(resp) => {
                                *d_hb.last_error.write().await = None;
                                if !resp.session_active {
                                    d_hb.log(
                                        "⚠️ Session no longer active according to server".into(),
                                    )
                                    .await;
                                }
                            }
                            Err(shadowmesh_core::ShadowMeshError::TooManyRequests(msg)) => {
                                ...
                                *d_hb.last_error.write().await =
                                ...
                                let status_res =
                                    crate::run_blocking(move || d_hb_disc.vpn_manager.get_status())
                                        .await;

                                if let Ok(shadowmesh_core::ConnectionStatus::Connected) = status_res
                                {
                                    d_hb.log(
                                        "🔌 Proactively disconnecting due to device limit".into(),
                                    )
                                    .await;
    ...
    #[cfg(unix)]
    {
        ...
        // Least-privilege: 0o600 — only the daemon's uid may connect.
        // The IPC token gates COMMANDS, but a world-writable socket would
        // let any local user hijack or DoS the socket path itself.
        let _ = daemon.file_system.set_permissions_mode(SOCKET_PATH, 0o600);
...
fn start_background_tasks(daemon: Arc<Daemon>) {
    ...
    tokio::spawn(async move {
        loop {
            ...
            let status_res =
                crate::run_blocking(move || d_health_check.vpn_manager.get_status()).await;

            if matches!(status_res, Ok(status) if status == shadowmesh_core::ConnectionStatus::Connected || status == shadowmesh_core::ConnectionStatus::Degraded)
            {
    ...
    tokio::spawn(async move {
        loop {
            ...
            let expired_res =
                crate::run_blocking(move || d_p.vpn_manager.check_pause_expiry()).await;
                ...
                match tunnel.inner.try_wait() {
                    Ok(Some(status)) => {
                        ...
                        tokio::spawn(async move {
                            ...
                            let _ = crate::run_blocking(move || {
                                d_v.vpn_manager
                                    .set_status(shadowmesh_core::ConnectionStatus::Disconnected)
                            })
                            .await;
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            let stats_res = crate::run_blocking(move || d_stats.vpn_manager.get_stats()).await;

            if let Ok(stats) = stats_res {
                crate::metrics::VPN_BYTES_SENT
                    .inc_by(stats.bytes_sent.saturating_sub(crate::metrics::VPN_BYTES_SENT.get()));
                crate::metrics::VPN_BYTES_RECV.inc_by(
                    stats.bytes_received.saturating_sub(crate::metrics::VPN_BYTES_RECV.get()),
                );

                let d_status = Arc::clone(&d_healing);
                let status_res =
                    crate::run_blocking(move || d_status.vpn_manager.get_status()).await;
            ...
            if let Ok(Ok(report)) = detector_res {
                ...
                if !last_network_id.is_empty() && current_net_id != last_network_id {
                    ...
                    let status_res =
                        crate::run_blocking(move || d_healing_status.vpn_manager.get_status())
                            .await;
