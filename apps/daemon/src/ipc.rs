...
/// Constant-time IPC session-token comparison.
///
/// Compares SHA-256 digests of both sides (equal 32-byte length always —
/// no length leak) using `subtle`'s ct_eq: no early-exit byte comparison
/// leaks prefix matches over the local socket. SHA-256 (not a fast hasher)
/// because this is an authentication decision — digest collisions must be
/// cryptographically hard, not merely improbable.
pub fn ipc_token_eq(presented: &str, expected: &str) -> bool {
    use subtle::ConstantTimeEq;

    let digest = |s: &str| -> [u8; 32] {
        use sha2::{Digest, Sha256};
        let mut h = Sha256::new();
        // Token strings are short; single update is fine.
        h.update(s.as_bytes());
        h.finalize().into()
    };

    let d_presented = digest(presented);
    let d_expected = digest(expected);
    bool::from(d_presented.ct_eq(&d_expected))
}
    ...
    async move {
        ...
        loop {
            ...
            let response = match serde_json::from_slice::<VpnCommand>(&frame) {
                Ok(cmd) => {
                    // Big-Tech Standard: Compile-time stripping of security bypasses
                    ...
                    #[cfg(not(debug_assertions))]
                    {
                        if let Ok(expected) = std::env::var("SHADOWMESH_IPC_TOKEN") {
                            // Constant-time token comparison: an early-exit
                            // string compare leaks prefix-match timing over
                            // the local socket. Fixed-length dummy digest
                            // keeps the length-mismatch path uniform too.
                            if !ipc_token_eq(&cmd.token, &expected) {
                            ...
                            // In production, missing token is a fatal security error
                            error!("❌ Security Error: SHADOWMESH_IPC_TOKEN not set");
                            VpnResponse {
                                success: false,
                                message: "System Security Violation: IPC Lockdown".into(),
                                data: None,
                            }
                        }
                    }
        ...
        match cmd.action {
            ...
            VpnAction::Status => {
                ...
                if let Ok((stats, p_stats, status, traffic_pref, split_config)) = stats_res {
                    ...
                    let prev_sent =
                        daemon.stats.bytes_sent.swap(stats.bytes_sent, Ordering::SeqCst);
                    let prev_recv =
                    ...
                    // RFC-016 G3: the data-plane truth — what tunnel is
                    // actually running, independent of the UI state machine.
                    let tunnel = {
                        let guard = daemon.active_tunnel.lock().await;
                        match guard.as_ref() {
            ...
            VpnAction::GetLogs => {
                ...
                VpnResponse {
                    success: true,
                    message: "OK".into(),
                    data: Some(VpnResponseData::Logs(logs)),
                }
            }
            ...
            VpnAction::GetDiagnostics => daemon.handle_diagnostics().await,
            VpnAction::Activate { code } => daemon.handle_activate(code.into_owned()).await,
            VpnAction::Connect { node_id, mode } => {
                daemon.handle_connect(node_id.into_owned(), mode.map(|m| m.into_owned())).await
            }
            VpnAction::Disconnect => daemon.handle_disconnect().await,
            VpnAction::Pause { minutes } => {
                let d_v = daemon.vpn_manager.clone();
                let res = crate::run_blocking(move || d_v.pause(minutes)).await;
                ...
                match res {
                    Ok(Ok(_)) => {
                        ...
                        VpnResponse {
                            success: true,
                            message: format!("Paused for {}m", minutes),
                            data: None,
                        }
                    }
            ...
            VpnAction::SetKillSwitch { enabled } => {
                ...
                VpnResponse {
                    success,
                    message: format!("Kill Switch {}", if enabled { "Active" } else { "Inactive" }),
                    data: None,
                }
            }
            ...
            VpnAction::SetDnsOverHttps { enabled } => {
                ...
                VpnResponse {
                    success: true,
                    message: format!("DoH {}", if enabled { "Enabled" } else { "Disabled" }),
                    data: None,
                }
            }
            VpnAction::SetTrafficPreference { preference } => {
                ...
                VpnResponse {
                    success: true,
                    message: "Traffic Preference Updated".into(),
                    data: None,
                }
            }
            VpnAction::SetSplitTunnel { enabled, mode, apps } => {
                ...
                let _ = crate::run_blocking(move || {
                    d_v.set_split_tunnel_config(shadowmesh_core::SplitTunnelConfig {
                        enabled,
                        mode: split_mode,
                        app_list: apps_owned,
                    })
            ...
            VpnAction::SecureToken { op } => match op {
                SecureTokenOp::Get { key } => {
                    let val =
                        daemon.secure_storage.get_password("org.shadowmesh.tokens", &key).ok();
                    VpnResponse {
                        success: true,
                        message: "OK".into(),
                        data: val.map(VpnResponseData::Token),
                    }
                }
                SecureTokenOp::Set { key, value } => {
                    match daemon.secure_storage.set_password("org.shadowmesh.tokens", &key, &value)
                    {
                        Ok(_) => {
                            VpnResponse { success: true, message: "Stored".into(), data: None }
                        }
                        Err(e) => VpnResponse {
                            success: false,
            ...
            VpnAction::QrAuth { op } => match op {
                QrAuthOp::Authorize { token } => {
                    match daemon.api_client.qr_authorize(token.into_owned()).await {
                        Ok(()) => VpnResponse {
                            success: true,
                            message: "Pairing approved".into(),
                            data: None,
                        },
                        Err(e) => VpnResponse {
                            success: false,
                ...
                QrAuthOp::Generate => {
                    ...
                    match daemon
                        ...
                        .await
                    {
                        Ok(token) => VpnResponse {
                            success: true,
                            message: "OK".into(),
                            data: Some(VpnResponseData::QrToken { token }),
                        },
                        Err(e) => VpnResponse {
                            success: false,
                ...
                QrAuthOp::CheckStatus { token } => {
                    // RFC-018: the full session carries the minted activation
                    // credential once an authorized device approves. On
                    // approval we run the REAL activation path (device slot,
                    // plan binding) — never store the session uuid as a
                    // credential.
                    match daemon.api_client.qr_status_full(token.into_owned()).await {
                        Ok(session) if session.status == "authorized" => {
                            let Some(activation_code) = session.activation_code else {
                                return VpnResponse {
                                    success: false,
                                    message: "Pairing authorized but no credential was issued"
                                        .into(),
                                    data: None,
                                };
                            };
                            let activation = daemon.handle_activate(activation_code).await;
                            VpnResponse {
                                success: activation.success,
                                message: if activation.success {
                                    "Pairing activated".into()
                                } else {
                                    activation.message
                                },
                                data: Some(VpnResponseData::Generic(serde_json::json!({
                                    "status": "authorized",
                                }))),
                            }
                        }
                        Ok(session) => VpnResponse {
                            success: true,
                            message: "OK".into(),
                            data: Some(VpnResponseData::Generic(serde_json::json!({
                        ...
                        Err(e) => VpnResponse {
                            success: false,
                            message: format!("QR status check failed: {e}"),
                            data: None,
                        },
                    }
                }
            ...
            VpnAction::Obfuscation { action, config } => match action.as_ref() {
                "start" => {
                    ...
                    daemon
                        .log(format!(
                            "🕵️ Starting Obfuscation (Shadowsocks/UDP2RAW)... Config: {}",
                            config_json
                        ))
                        .await;
                    daemon.config.rcu(|c| {
                ...
                _ => {
                    ...
                    VpnResponse {
                        success: true,
                        message: "OK".into(),
                        data: Some(VpnResponseData::Generic(
                            serde_json::json!({ "running": enabled, "method": "shadowsocks" }),
                        )),
                    }
                }
            },
            ...
            VpnAction::DuressPin { action, hash } => match action.as_ref() {
                ...
                _ => {
                    ...
                    VpnResponse {
                        success: true,
                        message: "OK".into(),
                        data: h.map(|v| VpnResponseData::Generic(serde_json::json!(v))),
                    }
                }
            },
            VpnAction::PanicWipe => {
                ...
                let _ = crate::run_blocking(move || {
                    logger.log_event(
                        SecurityEventType::PanicInitiated,
                        "User triggered forensic wipe".into(),
                        true,
                        None,
                    );
                })
                ...
                // Memory Purge: Zeroize config and buffers
                {
                    daemon.config.rcu(|_| {
                ...
                // Small delay to allow log flush and zeroize effectiveness
            ...
            VpnAction::Camouflage { enabled } => {
                ...
                VpnResponse {
                    success: true,
                    message: format!("Camouflage {}", if enabled { "Active" } else { "Inactive" }),
                    data: Some(VpnResponseData::Generic(serde_json::json!(enabled))),
                }
            }
            VpnAction::Shutdown => {
