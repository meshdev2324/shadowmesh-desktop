...
use shadowmesh_daemon::{
    ...
    VpnResponseData, VpnTunnel, process_command,
};
...
async fn test_ipc_activate() {
    let daemon = setup_test_daemon().await;
    let cmd = VpnCommand {
        action: VpnAction::Activate { code: "SHADOW-MESH-ACTI-VATE-25-CHARS".into() },
        token: "".into(),
    };
    ...
    assert_eq!(config.activation_code, Some("SHADOWMESHACTIVATE25CHARS".into()));
...
#[tokio::test]
async fn test_ipc_diagnostics() {
    let daemon = setup_test_daemon().await;
    let cmd = VpnCommand { action: VpnAction::GetDiagnostics, token: "".into() };
    let resp = process_command(cmd, daemon).await;

    assert!(resp.success);
    assert_eq!(resp.message, "Diagnostic Report Generated");

    if let Some(VpnResponseData::Generic(data)) = resp.data {
        assert_eq!(data["daemon_version"], "1.0.0-PRO");
        assert!(data["api_connectivity"].is_boolean());
        assert!(data["metrics"].is_object());
    } else {
        panic!("Expected Generic response data for diagnostics");
    }
}

#[tokio::test]
async fn test_ipc_activate_gateway_unreachable() {
    struct UnreachableApi;
    #[async_trait::async_trait]
    impl ShadowApi for UnreachableApi {
        async fn qr_authorize(&self, _: String) -> Result<(), shadowmesh_core::ShadowMeshError> {
            Ok(())
        }
        async fn ping_gateway(&self) -> Result<bool, shadowmesh_core::ShadowMeshError> {
            Ok(false)
    ...
    let runner = Arc::new(MockRunner);
    let daemon = shadowmesh_daemon::create_test_daemon(Arc::new(UnreachableApi), runner);
    ...
    assert!(!resp.success);
    assert!(resp.message.contains("API Gateway Unreachable"));
}

#[tokio::test]
async fn test_security_obfuscation_logic() {
    use shadowmesh_daemon::security::{deobfuscate_string, obfuscate_string};
    let original = "https://api.shadowmesh.org";
    let key = 0x42;
