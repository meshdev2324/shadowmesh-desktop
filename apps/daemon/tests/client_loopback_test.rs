...
impl ShadowApi for MockApi {
    async fn qr_authorize(&self, _token: String) -> Result<(), shadowmesh_core::ShadowMeshError> {
        Ok(())
    }
    ...
    async fn qr_status_full(
        &self,
        _token: String,
    ) -> Result<shadowmesh_core::api_client::QrPairingStatus, shadowmesh_core::ShadowMeshError>
    {
        // Simulates the state AFTER an authorized device approved the pair:
        // the session carries the minted member key (25 chars).
        Ok(shadowmesh_core::api_client::QrPairingStatus {
            status: "authorized".into(),
            activation_code: Some("MEMBTESTCODE1234567890ABC".into()),
        })
    }
...
#[tokio::test]
async fn qr_authorized_session_activates_through_the_real_activation_path() {
    let _ = tracing_subscriber::fmt::try_init();
    let daemon = mock_daemon();
    let (client_sock, daemon_sock) = tokio::net::UnixStream::pair().expect("socketpair");
    let (r, w) = tokio::io::split(daemon_sock);
    ...
    // The authorized session must flow through the REAL activation path
    // (device slot + plan binding), never store the session uuid as a
    // credential (the pre-RFC-018 bug).
    let resp = client
        .request(VpnAction::QrAuth {
            op: shadowmesh_daemon::types::QrAuthOp::CheckStatus { token: "session-uuid".into() },
        })
        .await
        .expect("check-status roundtrip");
    assert!(resp.success, "activation must succeed: {}", resp.message);
    assert_eq!(resp.message, "Pairing activated");

    // The activation's plan binding is visible in the daemon's status.
    let status = client.request(VpnAction::Status).await.expect("status");
    let data = match status.data.expect("status payload") {
        shadowmesh_daemon::types::VpnResponseData::Status(v) => v,
        other => panic!("expected Status payload, got {other:?}"),
    };
    assert_eq!(data["plan"], serde_json::json!("Pro"), "plan must come from the minted credential");
}
