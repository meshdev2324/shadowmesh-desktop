...
/// `server_names`: RFC-024 allowlist — empty = no SNI gate (legacy).
...
async fn reality_client_server_interop_roundtrip() {
    ...
    let port =
        spawn_reality_server(hex::encode(&priv_key), vec![short_id.clone()], echo_port, Vec::new())
            .await;
...
async fn reality_wrong_short_id_falls_back_to_masquerade() {
    ...
    let port =
        spawn_reality_server(hex::encode(&priv_key), vec![good_sid], echo_port, Vec::new()).await;
...
/// RFC-024: ClientHello SNI in `server_names` allowlist authenticates;
/// SNI outside the list falls back to the decoy (never a VLESS session).
#[tokio::test]
async fn reality_multi_sni_allowlist_gate() {
    let _ = tracing_subscriber::fmt::try_init();
    let (echo_port, echo_task) = spawn_echo().await;
    let (priv_key, pub_key) = x25519_pair();
    let short_id = hex::encode(shadowmesh_core::secure_random_bytes(8).expect("CSPRNG"));
    let allow = vec!["www.example.com".to_string(), "cloudflare.example".to_string()];

    let port =
        spawn_reality_server(hex::encode(&priv_key), vec![short_id.clone()], echo_port, allow)
            .await;

    // Allowed SNI A → full roundtrip.
    let sock = TcpStream::connect(("127.0.0.1", port)).await.expect("connect a");
    let mut tls = RealityTlsStream::connect(
        sock,
        &base64::engine::general_purpose::URL_SAFE_NO_PAD.encode(&pub_key),
        &short_id,
    ...
    .await
    .expect("allowlisted SNI A must authenticate");
    tls.write_app(b"multi-a").await.expect("write a");
    assert_eq!(tls.read_app().await.expect("read a").expect("payload"), b"multi-a");
    drop(tls);

    // Allowed SNI B (second name) → full roundtrip.
    let sock = TcpStream::connect(("127.0.0.1", port)).await.expect("connect b");
    let mut tls = RealityTlsStream::connect(
        sock,
        &base64::engine::general_purpose::URL_SAFE_NO_PAD.encode(&pub_key),
        &short_id,
    ...
    .await
    .expect("allowlisted SNI B must authenticate");
    tls.write_app(b"multi-b").await.expect("write b");
    assert_eq!(tls.read_app().await.expect("read b").expect("payload"), b"multi-b");
    drop(tls);

    // SNI C outside allowlist + correct short_id → must NOT become a VLESS session.
    let sock = TcpStream::connect(("127.0.0.1", port)).await.expect("connect c");
    let result = RealityTlsStream::connect(
        sock,
        &base64::engine::general_purpose::URL_SAFE_NO_PAD.encode(&pub_key),
        &short_id,
    ...
    assert!(
        result.is_err(),
        "SNI outside server_names must fall back to decoy, never authenticate"
    );
