...
/// Test destination for the relayed "internet" endpoints: an RFC1918 local
/// interface address. The dispatcher's destination guard deliberately blocks
/// loopback (SSRF-to-self), so echo servers must bind a real LAN IP —
/// docker0 (172.17.0.1) exists on every Linux dev box running these tests
/// and is in the guard's allowed private range.
const TEST_ECHO_HOST: &str = "172.17.0.1";
...
async fn spawn_echo() -> (u16, tokio::task::JoinHandle<()>) {
    let listener = TcpListener::bind((TEST_ECHO_HOST, 0)).await.expect("echo bind");
...
async fn spawn_udp_echo() -> SocketAddr {
    let sock = UdpSocket::bind((TEST_ECHO_HOST, 0)).await.expect("udp bind");
...
async fn tcp_roundtrip(outbound: &dyn OutboundDialer, echo_port: u16, payload: &[u8]) {
    // Destination the relayed stream must reach: the echo host (RFC1918 —
    // allowed by the destination guard, unlike loopback).
    let dest = Endpoint::new_ip(TEST_ECHO_HOST.parse().unwrap(), echo_port);
...
async fn edge_system_trojan_tls_roundtrip() {
    ...
    let port = reserve_port().await;
    // Ephemeral per-run credential: unique each execution (scanner hygiene —
    // no literal credential-like values in test source).
    let trojan_password = uuid::Uuid::new_v4().to_string();
    let raw = serde_json::json!({
        "inbounds": [ { "tag": "trojan-tls", "protocol": "trojan",
            "listen": "127.0.0.1", "port": port,
            "settings": { "password": trojan_password,
...
async fn edge_system_vless_handshake_split_across_segments() {
    ...
    let echo_ip: std::net::Ipv4Addr = TEST_ECHO_HOST.parse().unwrap();
    header.extend_from_slice(&echo_ip.octets());
...
/// Fail-closed TLS: a hysteria inbound without cert_path/key_path must be a
/// hard config error — the listener may never serve bundled test credentials.
#[tokio::test]
async fn edge_config_hysteria_without_cert_paths_fails_closed() {
    let raw = serde_json::json!({
        "inbounds": [
            {
                "tag": "hysteria-in",
                ...
                "listen": "127.0.0.1",
                "port": 36712,
                "settings": { "password": uuid::Uuid::new_v4().to_string() }
            }
        ],
        "outbounds": [
            { "tag": "direct", "protocol": "direct", "settings": {} }
    ...
    assert!(
        config.validate_strict().is_err(),
        "hysteria inbound without cert_path/key_path must fail validation, never boot with test credentials"
    );
}
