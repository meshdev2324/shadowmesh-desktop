...
/// Spawns a UDP echo server on an ephemeral port. Binds an RFC1918 local
/// interface (docker0) rather than loopback — the dispatcher's destination
/// guard deliberately blocks relayed loopback traffic (SSRF-to-self), so a
/// loopback echo would be correctly rejected by the G2 reply path.
async fn spawn_udp_echo() -> std::net::SocketAddr {
    let sock = UdpSocket::bind("172.17.0.1:0").await.expect("bind echo");
