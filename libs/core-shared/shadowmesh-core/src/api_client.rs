...
/// RFC-018: full QR pairing session state.
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]
pub struct QrPairingStatus {
    /// "pending" | "authorized" | "expired"
    pub status: String,
    /// The member token minted at authorize time (present once authorized).
...
impl ApiClient {
    ...
    /// Fetches the full QR pairing session (RFC-018): status plus the
    /// activation credential when an authorized device has approved it.
    pub fn qr_status_full(&self, token: String) -> Result<QrPairingStatus, ShadowMeshError> {
        get_runtime()?.block_on(self.qr_status_full_async(token))
    }

    /// Full QR pairing session (Async).
    pub async fn qr_status_full_async(
        &self,
        token: String,
    ) -> Result<QrPairingStatus, ShadowMeshError> {
        let url = format!("{}/api/v1/auth/qr/status/{}", self.base_url, token);
        let request = self.client.get(&url);
        let request = self.add_headers(request).await;
        let response = request.send().await.map_err(|_| ShadowMeshError::ConnectionFailed)?;
        if !response.status().is_success() {
            return Err(ShadowMeshError::Other("QR status check failed".to_string()));
        }
        let body: serde_json::Value = response
            .json()
            .await
        ...
        Ok(QrPairingStatus {
            ...
            activation_code: body["issued_token"].as_str().map(|s| s.to_string()),
        })
    }
