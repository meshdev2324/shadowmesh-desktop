...
/// Settings for the `tuic` outbound (TUIC v5, RFC-023).
#[derive(Debug, Clone, Deserialize)]
#[serde(deny_unknown_fields, rename_all = "snake_case")]
pub struct TuicSettings {
    pub server: String,
    pub port: u16,
    /// Client UUID string — parsed and rejected at config load, never at
    /// first dial.
    pub uuid: String,
    /// Raw password; used only as the TLS-exporter context for the
    /// session-bound Authenticate token.
    pub password: String,
    /// TLS SNI; defaults to `server` when omitted.
    #[serde(default = "default_none")]
    pub sni: Option<String>,
    /// Accept self-signed edge certificates — explicit operator choice only,
    /// never a default.
    #[serde(default)]
    pub insecure: bool,
}
...
pub struct VlessRealityServerSettings {
    ...
    /// RFC-024 multi-SNI allowlist (hostnames, no port). Absent/empty =
    /// no SNI gate (legacy). Present = ClientHello SNI must match
    /// (case-insensitive) or the handshake falls back to `sni_target`.
    #[serde(default)]
...
pub struct HysteriaInboundSettings {
    pub password: String,
    /// PEM certificate chain for the QUIC (Hysteria 2) TLS layer.
    /// Required — the inbound fails closed rather than serving bundled
    /// test credentials.
    pub cert_path: String,
...
/// Settings for the `tuic` inbound (TUIC v5 server role, RFC-023 phase 2).
#[derive(Debug, Clone, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct TuicInboundSettings {
    /// Authorized client UUID — parsed and rejected at config load.
    pub uuid: String,
    /// Raw password; used server-side as the TLS-exporter context to verify
    /// the Authenticate token in constant time.
    pub password: String,
    /// PEM certificate chain for the QUIC TLS layer. Required — the inbound
    /// fails closed rather than serving bundled test credentials.
    pub cert_path: String,
    /// PEM private key matching `cert_path`. Required.
    pub key_path: String,
}
...
mod tests {
    ...
    #[test]
    fn tuic_valid_parses_with_defaults() {
        let raw = serde_json::json!({
            "server": "edge.example.org",
            "port": 443,
            "uuid": "2dd9d878-0b3f-4a1e-9c2d-7f6a5b4c3d2e",
            "password": "hunter2",
        });
        let s: TuicSettings = parse_strict(&raw, "tuic").unwrap();
        assert_eq!(s.port, 443);
        assert_eq!(s.sni, None, "sni defaults to server at construction");
        assert!(!s.insecure, "insecure must never default on");
    }

    #[test]
    fn tuic_rejects_unknown_field() {
        let raw = serde_json::json!({
            "server": "edge.example.org",
            "port": 443,
            "uuid": "2dd9d878-0b3f-4a1e-9c2d-7f6a5b4c3d2e",
            "password": "pw",
            "congestion": "bbr", // not part of the v1 schema
        });
        assert!(parse_strict::<TuicSettings>(&raw, "tuic").is_err());
    }

    #[test]
    fn tuic_rejects_missing_uuid() {
        let raw =
            serde_json::json!({ "server": "edge.example.org", "port": 443, "password": "pw" });
        assert!(parse_strict::<TuicSettings>(&raw, "tuic").is_err());
    }

    #[test]
    fn tuic_sni_and_insecure_parse() {
        let raw = serde_json::json!({
            "server": "10.0.0.1",
            "port": 8443,
            "uuid": "2dd9d878-0b3f-4a1e-9c2d-7f6a5b4c3d2e",
        ...
        let s: TuicSettings = parse_strict(&raw, "tuic").unwrap();
        assert_eq!(s.sni.as_deref(), Some("cdn.example.org"));
        assert!(s.insecure);
    }
    ...
    fn vless_inbound_with_reality_parses() {
        ...
        let raw = serde_json::json!({
            ...
            "decoy": "security.ubuntu.com:443",
            "reality": {
                "private_key": hex64,
                "short_ids": ["0123456789abcdef"],
                "sni_target": "security.ubuntu.com:443",
                "server_names": ["security.ubuntu.com", "www.cloudflare.com"]
            }
        });
        ...
        assert_eq!(
            r.server_names,
            vec!["security.ubuntu.com".to_string(), "www.cloudflare.com".to_string()]
        );
        assert_eq!(s.decoy.as_deref(), Some("security.ubuntu.com:443"));
    }

    #[test]
    fn vless_inbound_reality_server_names_defaults_empty() {
        // G6: missing server_names must parse (legacy configs / no SNI gate).
        let hex64: String = uuid::Uuid::new_v4().simple().to_string()
            + uuid::Uuid::new_v4().simple().to_string().as_str();
        let raw = serde_json::json!({
            "uuid": "01234567-89ab-cdef-0123-456789abcdef",
        ...
        let s: VlessInboundSettings = parse_strict(&raw, "vless-inbound").unwrap();
        let r = s.reality.expect("reality present");
        assert!(r.server_names.is_empty(), "G6: absent server_names ⇒ empty (no gate)");
    }
    ...
    #[test]
    fn hysteria_inbound_requires_cert_paths() {
        // Missing cert_path/key_path must be a hard error — the inbound
        // fails closed rather than serving bundled test credentials.
        let raw = serde_json::json!({ "password": "pw" });
        assert!(parse_strict::<HysteriaInboundSettings>(&raw, "hysteria-inbound").is_err());
    }

    #[test]
    fn hysteria_inbound_with_cert_paths_parses() {
        let raw = serde_json::json!({
            "password": "pw",
            "cert_path": "/etc/shadowmesh/edge-hysteria-cert.pem",
            "key_path": "/etc/shadowmesh/edge-hysteria-key.pem"
        });
        let s: HysteriaInboundSettings = parse_strict(&raw, "hysteria-inbound").unwrap();
        assert_eq!(s.cert_path, "/etc/shadowmesh/edge-hysteria-cert.pem");
        assert_eq!(s.key_path, "/etc/shadowmesh/edge-hysteria-key.pem");
    }

    #[test]
    fn hysteria_inbound_rejects_unknown_field() {
        let raw = serde_json::json!({
            "password": "pw",
            "cert_path": "/etc/c.pem",
            "key_path": "/etc/k.pem",
    ...
    #[test]
    fn tuic_inbound_requires_cert_paths() {
        let raw = serde_json::json!({
            "uuid": "2dd9d878-0b3f-4a1e-9c2d-7f6a5b4c3d2e",
            "password": "pw"
        });
        assert!(parse_strict::<TuicInboundSettings>(&raw, "tuic-inbound").is_err());
    }

    #[test]
    fn tuic_inbound_with_cert_paths_parses() {
        let raw = serde_json::json!({
            "uuid": "2dd9d878-0b3f-4a1e-9c2d-7f6a5b4c3d2e",
            "password": "pw",
            "cert_path": "/etc/shadowmesh/edge-tuic-cert.pem",
            "key_path": "/etc/shadowmesh/edge-tuic-key.pem"
        });
        let s: TuicInboundSettings = parse_strict(&raw, "tuic-inbound").unwrap();
        assert_eq!(s.cert_path, "/etc/shadowmesh/edge-tuic-cert.pem");
        assert_eq!(s.key_path, "/etc/shadowmesh/edge-tuic-key.pem");
    }

    #[test]
    fn tuic_inbound_rejects_unknown_field() {
        let raw = serde_json::json!({
            "uuid": "2dd9d878-0b3f-4a1e-9c2d-7f6a5b4c3d2e",
            "password": "pw",
            "cert_path": "/etc/c.pem",
