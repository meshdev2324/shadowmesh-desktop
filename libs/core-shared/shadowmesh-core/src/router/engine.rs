...
pub struct RoutingPipeline {
    ...
    /// Tag used when no rule matches (config `routing.default_outbound`).
    /// `None` preserves the legacy bypass semantics.
    default_outbound: Option<String>,
}

impl RoutingPipeline {
    pub fn new(rules: Vec<RoutingRule>) -> Self {
        Self { rules, default_outbound: None }
    }

    /// Builder-style default: the tag the config names as `default_outbound`.
    /// Without it, unmatched traffic falls through to the dispatcher's
    /// legacy bypass (which requires a literal "direct" outbound).
    pub fn with_default_outbound(mut self, tag: Option<String>) -> Self {
    ...
    pub async fn route(&self, context: &mut ConnectionContext) -> Result<Action> {
        ...
        // Default Action when no rule matches: honor the configured
        // default_outbound if present, else legacy bypass. (Bug fix: the
        // config field was previously dead — clients whose default is a
        // proxy outbound failed with "Default outbound 'direct' not found".)
        match &self.default_outbound {
            Some(tag) => Ok(Action::Route(tag.clone())),
            None => Ok(Action::Bypass),
        }
    }
