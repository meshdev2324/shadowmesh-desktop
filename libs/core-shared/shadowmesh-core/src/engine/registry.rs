...
pub struct ConnectionRegistry {
    ...
    /// Connections opened since process start (monotonic counter).
...
impl ConnectionRegistry {
    ...
    pub fn register(&self, metadata: ConnectionMetadata) -> Arc<ConnectionInfo> {
        ...
        // Observability (Command Center): every dispatch is visible on the
        // metrics endpoint even when the per-connection gauge is short-lived.
        metrics::counter!("shadowmesh_connections_total").increment(1);
    ...
    pub fn get_stats(&self) -> (u32, u64, u64) {
        ...
        // Gauges reflect the registry's point-in-time view so the Prometheus
        // endpoint and any API consumer report identical numbers.
        metrics::gauge!("shadowmesh_connections_active").set(active as f64);
        metrics::gauge!("shadowmesh_bytes_uploaded_total")
            .set((self.total_upload.load(Ordering::SeqCst) + current_up) as f64);
        metrics::gauge!("shadowmesh_bytes_downloaded_total")
            .set((self.total_download.load(Ordering::SeqCst) + current_down) as f64);
        (
            active,
