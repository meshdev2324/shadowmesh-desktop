...
impl ShadowMeshSystem {
    pub async fn new(config: Config) -> Result<Self> {
        ...
        let pipeline = Arc::new(
            crate::router::engine::RoutingPipeline::new(config.routing.rules.clone())
                .with_default_outbound(Some(config.routing.default_outbound.clone())),
        );
        let dispatcher =
            Arc::new(Dispatcher::new(registry.clone(), pipeline, Arc::new(dns_router), outbounds));

        // Bounded: connection floods apply backpressure instead of unbounded memory growth (protocol-rules §13).
    ...
    async fn init_outbounds(config: &Config) -> Result<Arc<OutboundRegistry>> {
        use crate::config::settings::{
            self, DirectSettings, ShadowsocksSettings, TrojanSettings, TuicSettings, VlessSettings,
        ...
        for cfg in &config.outbounds {
                ...
                .as_str()
            {
                ...
                "tuic" => {
                    let s = settings::parse_strict::<TuicSettings>(raw, &cfg.protocol)?;
    ...
    async fn init_inbounds(
        ...
        use crate::config::settings::{
            ...
            TuicInboundSettings, VlessInboundSettings, VmessInboundSettings,
        };
            ...
            match cfg.protocol.as_str() {
                ...
                "tuic" => {
                    let addr = format!(
                        "{}:{}",
                        cfg.listen.as_deref().unwrap_or("0.0.0.0"),
                        cfg.port.unwrap_or(8443)
                    );
                    let s: TuicInboundSettings =
                        settings::parse_strict(require_settings()?, &cfg.protocol)?;
                    let inbound = Arc::new(crate::transport::inbound::TuicInbound::new(
                        cfg.tag.clone(),
                        addr,
                        &s.uuid,
                        &s.password,
