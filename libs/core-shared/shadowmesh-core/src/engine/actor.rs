        ...
        while let Ok(event) = self.event_rx.recv().await {
            match event {
                EngineEvent::NewStream { context, stream } => {
                    metrics::counter!("shadowmesh_engine_events_total", "kind" => "new_stream")
                        .increment(1);
                ...
                EngineEvent::UdpPacket { context, payload, source, reply } => {
                    metrics::counter!("shadowmesh_engine_events_total", "kind" => "udp_packet")
                        .increment(1);
                    let dispatcher = self.dispatcher.clone();
                    tokio::spawn(async move {
                        let ctx = {
                ...
                EngineEvent::ConnectionInitiated { .. } => {
                    metrics::counter!("shadowmesh_engine_events_total", "kind" => "conn_initiated")
                        .increment(1);
                    debug!("Connection initiated event received");
                }
                EngineEvent::ConnectionEstablished { id, outbound_tag } => {
                    metrics::counter!("shadowmesh_engine_events_total", "kind" => "conn_established")
                        .increment(1);
                    debug!("Connection {} established via {}", id, outbound_tag);
                }
                EngineEvent::ConnectionClosed { id, tx_bytes, rx_bytes, reason } => {
                    metrics::counter!("shadowmesh_engine_events_total", "kind" => "conn_closed")
                        .increment(1);
                    info!(
