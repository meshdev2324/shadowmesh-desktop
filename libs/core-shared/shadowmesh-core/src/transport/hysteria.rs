...
pub struct HysteriaInbound {
    ...
    /// PEM certificate chain for the QUIC TLS layer. Required: the server
    /// refuses to serve without operator-provided key material.
...
impl HysteriaInbound {
    pub fn new(
        ...
        Self { tag, listen_addr, auth_password, cert_path, key_path, engine }
    }

    /// Loads the configured PEM cert/key pair into a rustls server config.
    /// Follows the same load pattern as `transport::inbound::tls_util`
    /// (fail-closed on missing/unparsable files).
    fn load_server_crypto(&self) -> Result<rustls::ServerConfig> {
        let (cert_path, key_path) = match (&self.cert_path, &self.key_path) {
            (Some(c), Some(k)) => (c, k),
            _ => {
                // Fail closed: never fall back to bundled test credentials.
                return Err(anyhow::anyhow!(
                    "Hysteria inbound '{}': TLS certificate not configured (cert_path/key_path required) — refusing to serve the committed test certificate",
                    self.tag
                ));
            }
        };
        ...
        let cert_file = std::fs::File::open(cert_path).map_err(|e| {
            anyhow::anyhow!(
                "Hysteria inbound '{}': open certificate file '{cert_path}': {e}",
                self.tag
            )
        })?;
        let certs: Vec<_> = rustls_pemfile::certs(&mut std::io::BufReader::new(cert_file))
            ...
            .map_err(|e| {
                anyhow::anyhow!(
                    "Hysteria inbound '{}': parse certificates from '{cert_path}': {e}",
                    self.tag
                )
            })?;
        if certs.is_empty() {
            return Err(anyhow::anyhow!(
                "Hysteria inbound '{}': no certificates found in '{cert_path}'",
                self.tag
            ));
        }

        let key_file = std::fs::File::open(key_path).map_err(|e| {
            anyhow::anyhow!("Hysteria inbound '{}': open key file '{key_path}': {e}", self.tag)
        })?;
        let key = rustls_pemfile::private_key(&mut std::io::BufReader::new(key_file))
            .map_err(|e| {
                anyhow::anyhow!(
                    "Hysteria inbound '{}': parse private key from '{key_path}': {e}",
                    self.tag
                )
            })?
            .ok_or_else(|| {
                anyhow::anyhow!(
                    "Hysteria inbound '{}': no private key found in '{key_path}'",
                    self.tag
                )
            })?;

        rustls::ServerConfig::builder().with_no_client_auth().with_single_cert(certs, key).map_err(
            |e| {
                anyhow::anyhow!(
                    "Hysteria inbound '{}': invalid TLS certificate/key pair: {e}",
                    self.tag
                )
            },
        )
...
impl InboundListener for HysteriaInbound {
    ...
    async fn listen(&self) -> Result<()> {
        ...
        // TLS key material is operator-provided (PEM files on disk). There is
        // deliberately no fallback: when cert_path/key_path are absent the
        // listener errors out instead of serving a committed test identity.
...
async fn handle_hysteria_connection(
    ...
    // Constant-time password check: a `!=` on strings short-circuits on the
    // first differing byte, leaking a prefix-match oracle through auth
    // latency. When lengths differ we still run a full-length CT comparison
    // against a dummy of the expected size (result forced false afterwards)
    // so the duration reveals neither the prefix nor the length relation.
    let password_ok = {
        use subtle::ConstantTimeEq;
        let received = received_password.as_bytes();
        let expected = auth_password.as_bytes();
        let lengths_match = received.len() == expected.len();
        let candidate: &[u8] = if lengths_match { received } else { expected };
        lengths_match && bool::from(candidate.ct_eq(expected))
    };
    if auth_type != 0x01 || !password_ok {
    ...
    // NOTE (deferred, not a TODO): Hysteria 2 here carries WireGuard-tunnel
    // (IP-level) framing, so there is no per-connection SOCKS-style
    // destination header to extract at accept time. The synthetic
    // "shadowmesh.local:0" endpoint is a placeholder; per-packet routing
    // metadata is enriched later from the inner IP headers at dispatch.
    // A real redesign (per-packet metadata plumbing) is out of scope here.
...
mod tests {
    ...
    async fn test_hysteria2_handshake_success() {
        let port = 18393; // Use different port
                          // Ephemeral per-run credential: unique each execution (scanner
                          // hygiene — no literal credential-like values in test source).
        let password = uuid::Uuid::new_v4().to_string();

        // Start mock server in background
        let server_password = password.clone();
        let server_handle = tokio::spawn(async move {
            if let Err(e) = setup_mock_server(port, &server_password).await {
