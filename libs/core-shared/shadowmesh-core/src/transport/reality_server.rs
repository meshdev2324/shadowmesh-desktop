...
/// REALITY at all. Returns the (empty) fallback buffer so the caller still
/// relays to the masquerade target — an active prober must never learn that
/// the handshake failed for a server-side config reason.
///
/// `debug!` rather than `warn!` on purpose: a prober hammering the port
/// would otherwise trip one log line per attempt (log-flooding side channel).
fn config_error() -> Vec<u8> {
    debug!("REALITY: invalid server key material configured");
    Vec::new()
}
...
async fn handshake(
    ...
    // Constant-time short_id check: slice `==` short-circuits on the first
    // differing byte, leaking a short_id prefix-match oracle through the
    // handshake latency. Compare the full 8-byte padded slot in one CT pass
    // instead of testing the id bytes and the zero padding separately.
    let short_ok = config.short_ids.iter().any(|allowed| match hex::decode(allowed.trim()) {
        Ok(b) if !b.is_empty() && b.len() <= 8 => {
            let mut expected = [0u8; 8];
            expected[..b.len()].copy_from_slice(&b);
            bool::from(plain[8..16].ct_eq(&expected))
        }
        _ => false,
    });
    if !short_ok {
    ...
    // RFC-024: multi-SNI allowlist gate (after REALITY auth, before ServerHello).
    // Empty `server_names` = legacy (no SNI gate). Non-empty: ClientHello SNI
    // must match an entry (ASCII case-insensitive) or fall back to the decoy.
    if !config.server_names.is_empty() {
        let sni_ok = find_sni(exts).map(|client_sni| {
            config.server_names.iter().any(|allowed| allowed.eq_ignore_ascii_case(client_sni))
        });
        match sni_ok {
            Some(true) => debug!("REALITY server: ClientHello SNI in server_names allowlist"),
            Some(false) | None => {
                debug!("REALITY server: ClientHello SNI rejected — falling back to decoy");
                return Err(ch_record.clone());
            }
        }
    }
...
/// RFC 6066 server_name extension (type 0x0000): returns the host_name
/// from the first SNI entry, or `None` if absent/malformed/not host_name.
fn find_sni(exts: &[u8]) -> Option<&str> {
    let mut pos = 0usize;
    while pos + 4 <= exts.len() {
        let ext_type = u16::from_be_bytes([exts[pos], exts[pos + 1]]);
        let elen = u16::from_be_bytes([exts[pos + 2], exts[pos + 3]]) as usize;
