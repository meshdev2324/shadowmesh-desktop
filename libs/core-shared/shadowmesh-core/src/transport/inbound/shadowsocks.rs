...
//!
//! # Security: pre-auth handshake timeout
//!
//! The TCP address-parse phase runs *before* the engine authenticates the
//! destination stream, so a slowloris client that dribbles one byte at a
//! time could otherwise pin a task per connection indefinitely. The whole
//! address read is bounded by [`SS_PREAUTH_TIMEOUT`] (5 seconds), matching
//! the VLESS/VMess handshake windows.
...
/// Pre-auth window for the TCP target-address read: a legitimate client
/// delivers the whole address within round-trip time; a silent or
/// dribbling prober must not pin the handler task indefinitely
/// (slowloris guard, mirrors the VLESS/VMess handshake timeouts).
const SS_PREAUTH_TIMEOUT: std::time::Duration = std::time::Duration::from_secs(5);
...
impl ShadowsocksInbound {
    ...
    async fn handle_tcp(
        ...
        // We use a small buffer to parse it. The whole address read is
        // pre-auth, so it runs under one 5s deadline (see SS_PREAUTH_TIMEOUT).
        let destination = tokio::time::timeout(SS_PREAUTH_TIMEOUT, async {
            ...
            match atyp {
                ...
                3 => {
                    ...
                    let port = u16::from_be_bytes(port_buf);
                    Ok(Endpoint::new_domain(domain, port))
                }
                4 => {
                    // IPv6
        ...
        .await
        .map_err(|_| anyhow!("Shadowsocks handshake timed out"))??;
